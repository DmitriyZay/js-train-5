...
// Завдання 4: Дано масив рядків. Видаліть третій елемент масиву та перетворіть в рядок 
...
// Завдання 6: Функція приймає деструктуризований масив чисел 
//де буду перший елемент та масив всіх інших.
// За допомогою циклу for переберіть масив, знайдіть мінімальне та максимальне значення. 
...
let minNumber=[firstElement];
let maxNumber=[firstElement];

for (const i in restElements){
	if (minNumber>restElements[i]){
		minNumber=restElements[i];
	};
	if (maxNumber<restElements[i]){
		maxNumber=restElements[i];
	};
	};
...
function addTenToEach(array) {
  ...
  // Виводимо результат
let newArray=Array(array.lenght);
for(const i in array){
...
// Завдання 11: Функція приймає масив чисел. Поверніть новий масив, який містить тільки парні 
...
// Завдання 12: Функція приймає масив чисел. 
//Поверніть новий масив, в якому всі числа помножені на їх індекс.
...
function replaceNumbers(arr) {
  ...
  for (const i in arr){
	if (arr[i]>10){
		newArray[i]="Greater than 10";
	} else {
		newArray[i]="Less than or equal to 10";
	}
  };
  return newArray;
}
